...
def mean_pairwise(ordinal_variable, data, price):
    """
    ...
    """
    mean_prices = data.groupby(ordinal_variable)[price].mean()
...
def anova_categoricals_features(features, data, target="Sale price"):
    """
    ...
    for feature in features:
         ...
         # Drop missing values in feature or target
        subset = data[[target, feature]].dropna()

        if data[feature].nunique() < 2:  # Skip columns with only 1 unique value
          print(f'{feature}: skipped (only 1 unique value)')
          continue
